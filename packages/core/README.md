...
`ModuleValidationError`, `NoSuchFactoryError`, `NoSuchKeyError`, `SingletonDisposedDuringInitError`, and `FactoryReentrancyError` — each is documented at its definition in [`src/errors.ts`](src/errors.ts).
